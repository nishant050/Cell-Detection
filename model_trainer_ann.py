...
class Model_Trainer_Ann:
    ...
    def model_train(self, load_model = False, model_path = None, auto = False):
        ...
        if load_model:
            ...
            inputs = tf.keras.Input(shape=(self.processed_data.shape[1],))

            hidden_layer_1 = tf.keras.layers.Dense(500, activation='relu')(inputs)
            hidden_layer_2 = tf.keras.layers.Dense(300, activation = 'relu')(hidden_layer_1)
            hidden_layer_3 = tf.keras.layers.Dense(50, activation = 'relu')(hidden_layer_2)
            output_layer = tf.keras.layers.Dense(1, activation='sigmoid')(hidden_layer_3)

            # Compile the model
            self.model = tf.keras.Model(inputs=inputs, outputs=output_layer)
